...
.factory('mainFactory', ['$http', 
  ...
  .factory('monsterDatasource', ['$timeout', '$http',
    function ($timeout, $http) {
      ...
      var cf='';
      return {
        setFilter: function(nfilter,cfilter){nf = nfilter;cf = cfilter;},
        getUiScrollMonsters: function(pageData) {
          pageData = {"descriptor":pageData, "account": 1};
        ...
        if(typeof mod === 'object'){
          ...
          if(mod.bonus==""){
            var tmp = '<strong>' + actor + '</strong>, ' + mod.name + ": " + this.rollDice(damage) + "<br><span class='combatSpecial' contenteditable='false'>" + special + "<br>";
              ...
              var fullResult;
              if(mod.bonus != null){if(mod.bonus.substr(0,1) != "+" && mod.bonus.substr(0,1) != "-"){mod.bonus = "+"+mod.bonus;}}
              switch(adv){
                case 'a':
              ...
              if(damage){fullResult = "Hit <strong>AC" + fullResult + "</strong> for " + this.rollDice(damage) + " ";}
              if(special){fullResult = fullResult + "<br><span class='combatSpecial' contenteditable='false'>" + special + '</span> ';}
        ...
        if(typeof scenes[data.name] === 'undefined'){
          if(scenes.length>=5){alert("You can only play 5 scenes at once.");return;}
            ...
            for(var j=0;j<aScene.effects.length;j++){
              ...
              if(typeof aScene.nextPlay == "undefined"){aScene.nextPlay = 0;}

            unregister[data.name] = $rootScope.$watch(function(){return aScene.context.currentTime;}, function() {
              ...
              aScene.effects.forEach(function(Effect){
                ...
                if(typeof aScene === "undefined"){return;}
      ...
      chkScene: function(name){
        ...
        for(var j=0;j<aScene.effects.length;j++){
          if(aScene.effects[j].nextPlay<aScene.context.currentTime+0.5 && (aScene.effects[j].active || aScene.effects[j].active == null)){
            ...
            var perChance=0;
            for(var k=0;k<aScene.effects[j].sounds.length;k++){perChance += aScene.effects[j].sounds[k].chance;}
        ...
        if(aScene.context.currentTime < 2){startSceneFade = aScene.fadeIn;}
        ...
        aScene.gainNode[gainIndex].gain.linearRampToValueAtTime(aScene.gainNode[gainIndex].gainSet, (aScene.context.currentTime + (startSceneFade)));
          ...
          var channelplayed;
          if(aScene.context.destination.maxChannelCount< 3){(channelMax = 1)}
          if(aScene.effects[j].sounds[k].randLoc){
            for(var i=0;i<7;i++){silence.connect(aScene.merger, 0, i);}
            ...
            for(var i=0;i<8;i++){source.connect(aScene.merger, 0, i);}
            channelplayed = "all";
          }
