...
.controller('SoundEditCtrl', ['$scope', '$interval', 'soundsEditFactory', 'soundsFactory','$state', '$stateParams', '$filter',
  ...
  function mixToMono(buffer) {
      ...
      $scope.effect.id = $stateParams.id;
      ...
      if ($scope.effect.id !== 'new') {
        ...
        Object.defineProperty($scope, "preDelay", {
            get: function () {
                return $scope.stepsArray.indexOf($scope.effect.preDelay);
            },
            set: function (value) {
                $scope.effect.preDelay = $scope.stepsArray[value];
            }
        });
        $scope.slider = {
            "options": {
              "delay":{showSelectionBar:true,
                stepsArray:$scope.stepsArray,
                noSwitching: true},
              "preDelay":{showSelectionBar:true,
    ...
    $scope.sceneInit = function(){
      $scope.sceneContent = {};
      $scope.sceneContent.id = $stateParams.id;
    ...
    $scope.collectionInit = function(){
      ...
      $scope.collectionContent.id = $stateParams.id;
    ...
    $scope.addSound = function(){$scope.effect.sounds.push({});};
...
.controller('MonstersCtrl', ['$scope', '$log', 'monsterFactory', '$filter', '$timeout', '$state', '$stateParams', 
  function($scope, $log, monsterFactory, $filter, $timeout, $state, $stateParams) {
  ...
  $scope.tinymceOptionsWithButton = {
    ...
    toolbar: 'undo redo | bold italic | link | bullist numlist | code | collapseButton'
       ...
       if ($stateParams.id !== 'new') {
          ...
          var objectData = {"where":[{"id":$stateParams.id}]};
        ...
        $scope.currentMonster = monster;
          $state.go('editMonsters',{id:$scope.currentMonster.id},{notify:false, reload:false});
          //$state.href('editMonsters', {id:view.view}, {absolute:true});
      }
      ...
      $scope.addSkill = function(){
         console.log("Test");
       $scope.currentMonster.skills.push({"name": "", "bonus": ""});
      };

      $scope.addAttack = function(){
        console.log("Test");
        $scope.currentMonster.attacks.push({"name": "", "bonus": "", "damage": "", "special": ""});
      };

      $scope.addLA = function(){
        console.log("Test");
        $scope.currentMonster.legendaryActions.push({"name": "", "bonus": "", "damage": "", "special": ""});
      };
  ...
  $scope.addMulti = function(attack){
    if(typeof attack.submenu == 'undefined'){attack.submenu=[];}
    attack.submenu.push({"name":'',"bonus":'',"type":'ma'});
...
.controller('combatCtrl', ['$scope', '$compile', '$rootScope', 'filterFilter', '$filter','$http', 'monsterFactory', 'mainFactory', '$timeout', '$sce', 'localStorageService', 'soundsEditFactory', 
  ...
  $scope.selectedRow='';
  $scope.groups = [];
  $scope.groups.push({"name": "The Party","group": [{"name":"Savannah","turn":0},{"name":"Jason","turn":0},{"name":"Alex","turn":0},{"name":"Sarah","turn":0}]});
  ...
  if(localStorageService.get('combatants'))   $scope.combatants = localStorageService.get('combatants');
    else   $scope.combatants = [{"name":"Savannah","turn":0},{"name":"Jason","turn":0},{"name":"Alex","turn":0},{"name":"Sarah","turn":0}];
  ...
  if(localStorageService.get('diceBagQuick'))   $scope.diceBagQuick = localStorageService.get('diceBagQuick');
    else   $scope.diceBagQuick = [{"name":'d4', "roll": '1d4'},{"name":'d6', "roll": '1d6'},{"name":'d8', "roll": '1d8'},{"name":'d10', "roll": '1d10'},{"name":'d12', "roll": '1d12'},{"name":'d20', "roll": '1d20'},{"name":'d100', "roll": '1d100'},{"name":'6d6', "roll": '6d6'},{"name":'2x d20', "roll": '1d20 | 1d20'}];
    ...
    localStorageService.set('diceBagQuick', $scope.diceBagQuick);
        ...
        $scope.slider = {
            "options": {
              "percent":{showSelectionBar:true,floor:1,ceil:100,step:1,hideLimitLabels:true}
            }
        };
  ...
  $scope.addDie = function(die){
    $scope.diceBagQuick.push({"name":die.name,"roll": die.roll});
    $scope.new.roll='';
    $scope.new.name='';
    }
  ...
  $scope.deathCheck = function(hp,index){if(hp<0){$scope.combatants.splice(index,1);}};
  ...
  $scope.preAction = function(scope){
    ...
    if(typeof scope.subsubitem != "undefined"){
      ...
      var objectData = {"where":[{"name":scope.subsubitem.name}]};
      ...
      $scope.Ui.turnOff('combatantDropdown');
      $scope.combatants[$scope.selectedRow].catt = subitem;
      console.log(subitem);
    }
    else{
      $scope.Ui.turnOff('combatantDropdown');
  ...
  $scope.Action = function(actor,mod,adv){
    adv = adv || "";
    if(mod == null){mod = {bonus:"+0",name:"Straight d20"+adv};}
    console.log(mod);
    if(mod.type == "ma"){
    ...
    if(mod.type == "ca"){
      console.log($scope.combatants[$scope.selectedRow]);
              ...
              if(damage.substring(0,8) == '#summon#'){
                ...
                var combattxt = {"name":mod.name,"bonus":attack,"damage":damage,"special":$scope.combatants[$scope.selectedRow].spell.shorttext};
              }
  ...
  $scope.order = function(e) {
    ...
    if($scope.combatants[0].img != "" &&  $scope.combatants[0].img !== undefined) mainFactory.setShowFile({"path":$scope.combatants[0].img})
    $timeout(function() { 
  ...
  $scope.startCombat = function(){
    for(var i in $scope.combatants){$scope.combatants[i].turn = 1;}
    $scope.order();
    };
  ...
  $rootScope.loadMonster = function(monster){
    ...
    var count = $scope.combatants.reduce(function(n, combatant) {return n + (combatant.name.substring(0, tempmon.name.length) === tempmon.name); }, 0);
...
.controller('SoundCtrl', ['$scope', '$interval','$timeout','$http', 'soundsFactory', 'soundsEditFactory', 
  ...
  function getRandomInt(min, max) {
    ...
    $scope.sceneOptions = function(scene){console.log($scope.$parent.sceneOpt = scene);$scope.Ui.turnOn('soundOptions');}

        var objectData = {"fields":"all"};
    soundsEditFactory.getScenes(objectData).then(
      function(response) {
        $scope.allScenes = response.data;
        soundsEditFactory.scenes = response.data;
...
.controller('SpellsCtrl', ['$rootScope', '$filter', '$window', 'filterFilter', '$scope','spellFactory', '$timeout', '$state', '$stateParams', '$sce' , 
  function($rootScope, $filter, $window, filterFilter, $scope, spellFactory, $timeout, $state, $stateParams, $sce){

        $scope.spell = {};
        $scope.spell.id = $stateParams.id;
    $scope.tinymceOptions = {
    onChange: function(e) {
    ...
    return (item.name.toString().indexOf($scope.filterName) > -1 && eval(item.level) == $scope.filterLevel)
        ...
        if ($scope.spell.id !== 'new') {
          ...
          var objectData = {"where":[{"id":$scope.spell.id}]};
        ...
        $scope.loadSpell = function(spell){
                       console.log(spell);
          $state.go('editSpells',{id:spell.id},{notify:false, reload:false});

          var objectData = {"where":[{"id":spell.id}]};
          spellFactory.getSpells(objectData).then(
              function(response) {
                $scope.heading = "Update Spell";
                       $scope.spell = response.data;
...
.controller('WorldCtrl', ['$rootScope', '$filter', '$scope','citizenFactory', 'hotRegisterer',
  function($rootScope, $filter, $scope, citizenFactory, hotRegisterer){
    ...
    $scope.init = function(){
      ...
      $scope.raceCol = [{data:'name',title: 'Race'},{data:'adulthood',title:'Adulthood'},{data:'middleAge',title:'Middle Age'},{data:'oldAge',title:'Old Age'},{data:'venerable',title:'Venerable'},{data:'maxAge',title:'Maximum Age'},{data:'friendRate',title:'Friend Rate'},{data:'enemyRate',title:'Enemy Rate'}];
      $scope.descCol = [{data:'type',title: 'Type'},{data:'text',title:'Text',width:460},{data:'gender',title:'Limit Gender (M/F)'}];
      $scope.profCol = [{data:'name',title:'Profession'},{data:'minAge',title:'Minimum Age Group'},{data:'maxAge',title:'Maximum Age Group'}];
    };

$scope.ageRegion = function(region,years,index){
  var pageData = {"region":region,"years":years};
  console.log(years);
...
.controller('NpcCtrl', ['$rootScope', '$filter', '$scope','citizenFactory', '$stateParams', function($rootScope, $filter, $scope, citizenFactory, $stateParams){
...
$scope.selectedCitizen = null;
  ...
  var objectData = {"region":$scope.region,"where":[{"id":citizen}]};
...
console.log($scope.selectedCitizen);
      citizenFactory.getWorld().then(
        function(response) {
        $scope.regions = response.data.regions;
         $scope.professions = response.data.professions;
  ...
  $scope.getAspects = function(){
    ...
    var pageData = {"region":$scope.region,"gender":gender};
  ...
  $scope.newCitizen = function(){
    ...
    $scope.father = [];
    $scope.children = [];
    $scope.selectedCitizen={"firstName":"","lastName":"","race":{},"age":"","gender":"","alive":"1","married":"0","spouse":"","children":[],"mother":"","father":"","friends":"","enemies":"","mannerisms":"","lineage":"","quirks":"","abilities":"","features":{"Special":"","Face Shape":"","Skin Complexion":"","Skin Color":"","Hair Description":"","Hair Color":"","Eye Description":"","Eye Color":"","Clothing":"","Body Type":""},"region":$scope.region,"generation":"NULL","profession":"","notes":"","birthYear":"NULL","residentCity":""};
  } 
   ...
   $scope.updateCitizen = function(citizen){
    ...
    var pageData = {"region":$scope.region,"citizen":citizen};
  ...
  $scope.citizens = {};
  var objectData = {"region":$scope.region};
    citizenFactory.getCitizens(objectData).then(
      function(response) {
        ...
        console.log($scope.citizens.alive);
        $scope.citizens.alive = orderBy($scope.citizens.alive, ['age','name'], false);
        for(var i in $scope.citizens.all){$scope.citizens.all[$scope.citizens.all[i].id]=$scope.citizens.all[i];}
      },
      function(err) {
        console.log(err);
      });    
