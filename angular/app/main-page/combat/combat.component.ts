...
setCombatPage(value) {
  ...
  private monsterService: MonsterService,
   ...
   changePage = (page: number) => {
        this.monsterService.getMonsters(this.monsterSearch, page).subscribe((result: any) => {
            this.pagerInfo = new PagerInfo(result);
            this.monsters = result.data;
        this.viewFavorites = false;
        });
...
fromEvent(this.input.nativeElement,'keyup')
    .pipe(
        ...
        tap((text) => {
           this.monsterService.getMonsters(this.input.nativeElement.value, 1).subscribe((result: any) => {
               this.pagerInfo = new PagerInfo(result);
                this.monsters = result.data;
                this.viewFavorites = false;
        });
...
loadCombatant = (combatant, order = true) => {
    ...
    if (this.recentMonsters.length > 5) this.recentMonsters.splice(0, this.recentMonsters.length - 5);
