...
export class FreeDraggingDirective implements AfterViewInit, OnDestroy {
  ...
  set scale(value)  {
    ...
    if (this.frameLocked) this.minBoundY = this.draggingBoundaryElement.offsetHeight - 150 - this.element.offsetTop;
    else this.minBoundY = this.draggingBoundaryElement.offsetHeight - 150  - this.element.offsetTop - (this.element.scrollHeight * this.scale);
...
constructor(
  ...
  ngAfterViewInit(): void {
	...
	initDrag(): void {
		...
		else this.minBoundY = 50 + this.element.offsetTop - (this.element.scrollHeight * this.scale);
		console.log({
			// pageX: $event.pageX,
			// pageY: $event.pageY,
			currentX: this.currentX,
