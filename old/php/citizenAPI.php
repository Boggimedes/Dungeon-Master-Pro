...
function getActiveCitizens($data = null){

		if(empty($data) || !isset($data)) return;
		$data = implode(",",$data);
		$this->citizens=[];
		$query  = "SELECT * FROM citizen WHERE id IN($data) AND account = $this->account";
		if ($result = $this->mysqli->query($query)) {
			while($row=$result->fetch_object()){
			                $row->features = json_decode($row->features);
		if(!is_object($row->features)) $row->features = json_decode($row->features);
		if($row->race && array_key_exists($row->race, $this->races)) $row->race 		= $this->races[$row->race];
...
function getCitizenDetails($data = null){
	...
	$where = null;
		...
		if(!is_object($this->citizen->features)) $this->citizen->features = json_decode($this->citizen->features);
