...
void (function addFilters() {
  const filters = Array.from(
    document.getElementById("filters").querySelectorAll("filter")
  );
...
function selectStyleElement() {
  ...
  styleElements
    ...
    .forEach((e) => (e.style.display = "none")); // hide all sections
  ...
  if (!["landmass", "ocean", "regions", "legend"].includes(sel)) {
    ...
    styleOpacityInput.value = styleOpacityOutput.value =
      el.attr("opacity") || 1;
  }
  ...
  if (
    ...
    styleStrokeWidthInput.value = styleStrokeWidthOutput.value =
  ...
  if (sel === "terrs") {
    ...
    styleHeightmapTerracingInput.value = styleHeightmapTerracingOutput.value =
      terrs.attr("terracing");
    styleHeightmapSkipInput.value = styleHeightmapSkipOutput.value =
      terrs.attr("skip");
    styleHeightmapSimplificationInput.value =
      styleHeightmapSimplificationOutput.value = terrs.attr("relax");
  ...
  if (sel === "terrain") {
    ...
    styleReliefSizeOutput.innerHTML = styleReliefSizeInput.value =
      terrain.attr("size");
    styleReliefDensityOutput.innerHTML = styleReliefDensityInput.value = 0.2; //terrain.attr("density");
    styleReliefSet.value = terrain.attr("set");
  }

  if (sel === "population") {
    stylePopulation.style.display = "block";
    stylePopulationRuralStrokeInput.value =
      stylePopulationRuralStrokeOutput.value = population
        .select("#rural")
        .attr("stroke");
    stylePopulationUrbanStrokeInput.value =
      stylePopulationUrbanStrokeOutput.value = population
        .select("#urban")
        .attr("stroke");
    styleStrokeWidth.style.display = "block";
    styleStrokeWidthInput.value = styleStrokeWidthOutput.value =
  ...
  if (sel === "regions") {
    ...
    styleStatesBodyOpacity.value = styleStatesBodyOpacityOutput.value =
      statesBody.attr("opacity") || 1;
    styleStatesBodyFilter.value = statesBody.attr("filter") || "";
    styleStatesHaloWidth.value = styleStatesHaloWidthOutput.value =
      statesHalo.attr("data-width") || 10;
    styleStatesHaloOpacity.value = styleStatesHaloOpacityOutput.value =
      statesHalo.attr("opacity") || 1;
    const blur =
      parseFloat(statesHalo.attr("filter")?.match(/blur\(([^)]+)\)/)?.[1]) || 0;
  ...
  if (sel === "legend") {
    ...
    styleLegendColItemsOutput.value = styleLegendColItems.value =
      el.attr("data-columns");
    styleLegendBackOutput.value = styleLegendBack.value = el
      .select("#legendBox")
      .attr("fill");
    styleLegendOpacityOutput.value = styleLegendOpacity.value = el
      .select("#legendBox")
      .attr("fill-opacity");
  ...
  if (sel === "ocean") {
    ...
    styleOceanFill.value = styleOceanFillOutput.value = oceanLayers
      .select("#oceanBase")
      .attr("fill");
    styleOceanPattern.value = document
      .getElementById("oceanicPattern")
      ?.getAttribute("href");
  ...
  if (sel === "temperature") {
    ...
    styleStrokeWidthInput.value = styleStrokeWidthOutput.value =
      el.attr("stroke-width") || "";
    styleTemperatureFillOpacityInput.value =
      styleTemperatureFillOpacityOutput.value = el.attr("fill-opacity") || 0.1;
    styleTemperatureFillInput.value = styleTemperatureFillOutput.value =
      el.attr("fill") || "#000";
    styleTemperatureFontSizeInput.value = styleTemperatureFontSizeOutput.value =
  ...
  if (sel === "armies") {
    ...
    styleArmiesFillOpacity.value = styleArmiesFillOpacityOutput.value =
      el.attr("fill-opacity");
  ...
  if (sel === "coastline" && styleGroupSelect.value === "sea_island") {
    ...
    const auto = (styleCoastlineAuto.checked = coastline
      .select("#sea_island")
      .attr("auto-filter"));
...
styleStrokeInput.addEventListener("input", function () {
  ...
  if (styleElementSelect.value === "gridOverlay" && layerIsOn("toggleGrid"))
    drawGrid();
});
...
styleFilterInput.addEventListener("change", function () {
  if (styleGroupSelect.value === "ocean")
    return oceanLayers.attr("filter", this.value);
...
styleTextureInput.addEventListener("change", function () {
  if (this.value === "none") texture.select("image").attr("xlink:href", "");
    ...
    getBase64(this.value, (base64) =>
      texture.select("image").attr("xlink:href", base64)
    );
});
...
function calculateFriendlyGridSize() {
  ...
  const friendly = `${rn(size * distanceScaleInput.value, 2)} ${
  ...
  $("#addFontDialog").dialog({
    ...
    buttons: {
      Add: function () {
        ...
        if (existingFont)
          return tip("The font is already added", false, "error");
...
function changeFontSize(size) {
  ...
  const getSizeOnScale = (element) => {
    ...
    if (element === "labels")
      return Math.max(rn((size + size / scale) / 2, 2), 1);
  ...
  $("#alert").dialog({
    ...
    buttons: {
      Apply: function () {
        ...
        getBase64(textureURL.value, (base64) =>
          texture.select("image").attr("xlink:href", base64)
        );
...
function applyStyleOnLoad() {
  ...
  const style =
    preset && (defaultStyles[preset] || localStorage.getItem(preset));
  ...
  if (preset && style && JSON.isValid(style)) {
    ...
    if (preset && preset !== "styleDefault" && ERROR)
      console.error(
        `Style preset ${preset} is not available in localStorage, applying default style`
      );
...
function changeStylePreset(preset) {
  if (customization)
    return tip("Please exit the customization mode first", false, "error");
  ...
  if (sessionStorage.getItem("styleChangeWarningShown")) {
    ...
    alertMessage.innerHTML =
      "Are you sure you want to change the style preset? All unsaved style changes will be lost";
    $("#alert").dialog({
      resizable: false,
  ...
  function changeStyle() {
    ...
    if (customPreset) {
      if (JSON.isValid(customPreset)) applyStyle(JSON.parse(customPreset));
        ...
        tip(
          "Cannot parse stored style JSON. Default style applied",
          false,
          "error",
          5000
        );
    ...
    removeStyleButton.style.display = stylePreset.selectedOptions[0].dataset
      .system
      ? "none"
      : "inline-block";
...
function addStylePreset() {
  ...
  const currentStyle =
    document.getElementById("stylePreset").selectedOptions[0].text;
  ...
  document
    .getElementById("styleSaverSave")
    .addEventListener("click", saveStyle);
  document
    .getElementById("styleSaverDownload")
    .addEventListener("click", styleDownload);
  document
    .getElementById("styleSaverLoad")
    .addEventListener("click", () => styleToLoad.click());
  document
    .getElementById("styleToLoad")
  ...
  function getStyle() {
    ...
    for (const selector in attributes) {
      ...
      for (const attr of attributes[selector]) {
        ...
        if (attr === "font-size" && el.hasAttribute("data-size"))
  ...
  function saveStyle() {
    if (!styleSaverJSON.value)
      return tip("Please provide a style JSON", false, "error");
    if (!JSON.isValid(styleSaverJSON.value))
      return tip(
        "JSON string is not valid, please check the format",
        false,
        "error"
      );
    if (!styleSaverName.value)
      return tip("Please provide a preset name", false, "error");
    if (styleSaverTip.innerHTML === "default")
      return tip(
        "You cannot overwrite default preset, please change the name",
        false,
        "error"
      );
  ...
  function styleDownload() {
    ...
    downloadFile(
      data,
      "style" + styleSaverName.value + ".json",
      "application/json"
    );
  }
  ...
  function styleUpload(dataLoaded) {
    if (!dataLoaded)
      return tip(
        "Cannot load the file. Please check the data format",
        false,
        "error"
      );
...
function removeStylePreset() {
  if (stylePreset.selectedOptions[0].dataset.system)
    return tip("Cannot remove system preset", false, "error");
