  ...
  const OceanLayers = function OceanLayers() {
    ...
    for (const i of cells.i) {
      ...
      const relaxed = chain.filter(
        (v, i) => !(i % relax) || vertices.c[v].some((c) => c >= pointsN)
      );
    ...
    function findStart(i, t) {
      if (cells.b[i])
        return cells.v[i].find((v) => vertices.c[v].some((c) => c >= pointsN)); // map border cell
      return cells.v[i][
        cells.c[i].findIndex((c) => cells.t[c] < t || !cells.t[c])
      ];
    }
