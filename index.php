...
if(strpos(__DIR__,"dev.gmapp")) $config['db']['dbname'] = "gm_master_tools_dev";
...
$app->map(['GET', 'POST'],'/register', function (Request $request, Response $response, $args) use($app) {
    if($request->getMethod()=="GET"){
        ...
        $param['racialBalance']= $row['racialBalance'];
            ...
            //Create Descriptives
        $query  = "INSERT INTO descriptives 
        ...
        SELECT 
            ...
            `descriptives`.`text`,
            `descriptives`.`gender`
        FROM `descriptives` WHERE account = 1";
        ...
        SELECT 
            ...
            `profession`.`maxAge`,
            '$check' AS `account`
        FROM `profession` WHERE account = 1";
...
$app->post('/api/monster/{action}', function (Request $request, Response $response) {
    ...
    switch($action){
        ...
        case 'getscroll':
            $response = $api->getScrollMonsters();
            $response = json_encode($response);
        break;
         case 'getui':
