...
function randomizeOptions() {
  ...
  neutralInput.value = neutralOutput.value = document.getElementById('epoch').value / 6000; //rn(1 + Math.random(), 1);
...
function regeneratePrompt() {
  ...
  const workingTime = (Date.now() - (last(mapHistory)? last(mapHistory).created : Date.now())) / 60000; // minutes
  if (workingTime < 5) {
