...
window.Markers = (function () {
  ...
  function getDefaultConfig() {
    ...
    return [
      { type: "volcanoes", icon: "🌋", multiplier: 0, fn: addVolcanoes },
      { type: "hot-springs", icon: "♨️", multiplier: 0, fn: addHotSprings },
      { type: "mines", icon: "⛏️", multiplier: 1, fn: addMines },
      { type: "bridges", icon: "🌉", multiplier: 0, fn: addBridges },
      { type: "inns", icon: "🍺", multiplier: 1, fn: addInns },
      {
        type: "lighthouses",
        icon: "#icon-lighthouse",
        intrinsicSize: 397.902,
        multiplier: 1,
        fn: addLighthouses,
      },
      {
      ...
      { type: "battlefields", icon: "⚔️", multiplier: 0, fn: addBattlefields },
      {
        type: "dungeons",
        icon: "#icon-dungeon",
        intrinsicSize: 32,
      ...
      { type: "hill-monsters", icon: "☠️", multiplier: 3, fn: addHillMonsters },
      {
      ...
      { type: "portals", icon: "🌀", multiplier: 0, fn: addPortals },
    ];
  }
  ...
  function generateTypes() {
    ...
    config.forEach(({ type, icon, multiplier, fn, intrinsicSize }) => {
      if (multiplier === 0) return;
      fn(type, icon, multiplier, intrinsicSize);
    });
  ...
  function addMarker({ cell, type, icon, dx, dy, px, size, intrinsicSize }) {
    const i = pack.markers.length;
    const [x, y] = getMarkerCoordinates(cell);
    const marker = { i, icon, type, x, y, cell, size, intrinsicSize };
    if (dx) marker.dx = dx;
    if (dy) marker.dy = dy;
    if (px) marker.px = px;
  ...
  function addVolcanoes(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
    ...
    if (!quantity) return;
    console.log("addVolcano");
    while (quantity) {
      const [cell] = extractAnyElement(mountains);
      const id = addMarker({ cell, icon, type, dx: 52, intrinsicSize });
  ...
  function addHotSprings(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
    ...
    while (quantity) {
      ...
      const id = addMarker({ cell, icon, type, dy: 52, intrinsicSize });
      const proper = Names.getCulture(cells.culture[cell]);
  ...
  function addMines(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
    ...
    while (quantity && hillyBurgs.length) {
      ...
      const id = addMarker({ cell, icon, type, intrinsicSize });
  ...
  function addBridges(type, icon, multiplier, intrinsicSize = null) {
    const { cells, burgs } = pack;
  ...
  function addInns(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
  ...
  function addLighthouses(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
  ...
  function addWaterfalls(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
    ...
    for (let i = 0; i < waterfalls.length && i < quantity; i++) {
      ...
      const id = addMarker({ cell, icon, type, size: 18, intrinsicSize });
      const proper = cells.burg[cell]
        ? pack.burgs[cells.burg[cell]].name
        : Names.getCulture(cells.culture[cell]);
      notes.push({
  ...
  function addBattlefields(type, icon, multiplier, intrinsicSize = null) {
  ...
  function addDungeons(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
    ...
    while (quantity) {
      ...
      const id = addMarker({ cell, icon, type, size: 25, intrinsicSize });
      ...
      notes.push({ id, name, legend, dungeonSeed });
      quantity--;
    }
  }

  function addLakeMonsters(type, icon, multiplier, intrinsicSize = null) {
    const { features } = pack;
  ...
  function addSeaMonsters(type, icon, multiplier, intrinsicSize = null) {
  ...
  function addHillMonsters(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;

    const hills = Array.from(
      cells.i.filter((i) => !occupied[i] && cells.h[i] >= 50 && cells.pop[i])
    );
    ...
    while (quantity) {
      ...
      const id = addMarker({ cell, icon, type, size, intrinsicSize });
  ...
  function addSacredMountains(type, icon, multiplier, intrinsicSize = null) {
  ...
  function addSacredForests(type, icon, multiplier, intrinsicSize = null) {
    const { cells, cultures } = pack;
  ...
  function addSacredPineries(type, icon, multiplier, intrinsicSize = null) {
    const { cells, cultures } = pack;
  ...
  function addSacredPalmGroves(type, icon, multiplier, intrinsicSize = null) {
    const { cells, cultures } = pack;
  ...
  function addBrigands(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
  ...
  function addPirates(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
  ...
  function addStatues(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
  ...
  function addRuins(type, icon, multiplier, intrinsicSize = null) {
    const { cells } = pack;
  ...
  function addPortals(type, icon, multiplier, intrinsicSize = null) {
    const { burgs } = pack;
    let portals = burgs
  ...
  return { generate, regenerate, getConfig, setConfig, occupied };
})();
