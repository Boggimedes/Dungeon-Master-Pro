...
function __construct($account, $region, $db) {
...
function getCitizensArray($data){
	...
	$where = null;
		...
		$this->getRegion($this->region);
		$query  = "SELECT firstName, lastName, gender, age, alive, race, profession, lineage, id FROM citizen WHERE region = {$this->region->id} AND account = $this->account $whereStr";
...
function seedRegion(){
		...
		foreach($racialBalance as $key=>$entry){
			for ($i = 0; $i < $entry->value; $i++) {
		        ...
		        if($lineage == "demigod" || $lineage == "fiend" || $lineage == "demon" || $lineage == "angel" )
	...
	if((mt_rand(1,1000) < $deathChance || $currentCitizen->age >= $currentCitizen->race->maxAge) && $currentCitizen->immortal != 1){
		...
		foreach($currentCitizen->children as $child){
 		...
 		//echo $this->sqlArray[count($this->sqlArray)-1]."<br>";
			...
			// 	}
			if(array_key_exists($child, $this->citizens)) $this->citizens[$child]->notes = $this->citizens[$child]->notes.$note;
			}
		} 
		else{
...
private function weddingBells(&$currentCitizen)  {
        ...
        foreach($this->wives as &$k){
            ...
            if($k->race->id != $currentCitizen->race->id && mt_rand(1,20) > 1) continue;
            $spouseOptions[]=&$k;
        }
...
function clearRegion($region){
	$query  = "DELETE FROM citizen WHERE account = '{$this->account}' AND region = $region AND excluded = 0";
...
function newCitizen($age, $race, $gender, $generation, $mother = null, $father = null, $lineage = null){
	        ...
	        $newCitizen->lineage			= $aspects['Lineage'];
...
function updateNPCRecord($recordType,$data){
	...
	foreach($data as $key=>$value){
		if($value === 0 || $value === 1 || $value == "NULL") $set .= ", `$key` = $value";
...
private function newChild(&$currentCitizen)  {
		...
		if($racialBalance == null) $racialBalance = 5;
       ...
       if($currentCitizen->immortal == 1) $birthRate = 3;
        ...
        foreach($this->fathers as &$k){
            ...
            if(in_array($k->id,$family)) continue;
            if($k->race->id != $currentCitizen->race->id && mt_rand(1,50) > 2) continue;
            $fatherOptions[]=&$k;
        }
        ...
        if($father->race->id == $currentCitizen->race->id){$childRace = $currentCitizen->race;}
                ...
                elseif(array_key_exists("Half-Celestial", $this->races)){
                	...
                	($father->race->id != $this->races["Half-Celestial"]->id && 
                			$currentCitizen->race->id == $this->races["Half-Celestial"]->id)){
			        	if(array_key_exists("Aasimar", $this->races)) $childRace = $this->races["Aasimar"]->id;
                ...
                elseif(array_key_exists("Half-Infernal", $this->races)){
                	...
                	($father->race->id != $this->races["Half-Infernal"]->id && 
                			$currentCitizen->race->id == $this->races["Half-Infernal"]->id)){
			        	if(array_key_exists("Tiefling", $this->races)) $childRace = $this->races["Tiefling"]->id;
		...
		if($father->lineage == "Lycanthrope" || $currentCitizen->lineage == "Lycanthrope")
			$childLineage = mt_rand(1,100) < 50? $childLineage = "Lycanthrope": $childLineage = null;

		if($father->lineage == "Lycanthrope" && $currentCitizen->lineage == "Lycanthrope")
			$childLineage = "Lycanthrope";

		if($father->lineage == "Demon" || $currentCitizen->lineage == "Demon"){
        	if(array_key_exists("Half-Infernal", $this->races)) $childRace = $this->races["Half-Infernal"]->id;
        		else $childLineage = "Half-Infernal";
			}

		if($father->lineage == "Fiend" || $currentCitizen->lineage == "Fiend"){
        	if(array_key_exists("Half-Infernal", $this->races)) $childRace = $this->races["Half-Infernal"]->id;
        		else $childLineage = "Half-Infernal";
			}

		if($father->lineage == "Angel" || $currentCitizen->lineage == "Angel"){
        	if(array_key_exists("Half-Celestial", $this->races)) $childRace = $this->races["Half-Celestial"]->id;
		...
		if($father->lineage == "Demigod" || $currentCitizen->lineage == "Demigod")
			$childLineage = mt_rand(1,100) < 50? $childLineage = "Demigod": $childLineage = null;

		if($father->lineage == "Deity" || $currentCitizen->lineage == "Diety")
