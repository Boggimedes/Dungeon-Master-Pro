...
function getMonsters($data){
	...
	if($_SESSION["settings"]->defaultMonsters) $monsterQuery = "(SELECT * FROM (SELECT * FROM monster WHERE account = $this->account
			UNION ALL SELECT * FROM monster WHERE account = 1) AS monster GROUP BY `name` ORDER BY `name`) AS monsters";
	 else $monsterQuery = "(SELECT * FROM monster WHERE account = $this->account) AS monsters";
...
function getScrollMonsters(){
	$nFilter = '';
	$cFilter = '';
	if($_SESSION["settings"]->defaultMonsters) $monsterQuery = "(SELECT name,img,cr,id FROM (SELECT name,img,cr,id FROM monster WHERE account = $this->account
			UNION ALL SELECT name,img,cr,id FROM monster WHERE account = 1) AS monster GROUP BY `name` ORDER BY `name`) AS monsters";
	 else $monsterQuery = "(SELECT name,img,cr,id FROM monster WHERE account = $this->account) AS monsters";
