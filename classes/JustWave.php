...
class JustWave
{
	...
	**/
	...
	private function init()
	{
		...
		if(isset($this->_OPTS['db'])) {
			$this->db = $this->_OPTS['db'];
		}
...
//		$subColor = imagecolorallocate($this->waveImg, (int)$r * 0.5 , (int)$g * 0.5, (int) $b * 0.5);
	...
	private function checkCache()
	{
		...
		try {
			// echo "test";
			// $this->db = $this->open($this->waveDir . "waveformDB.db");

			//new PDO('sqlite:' . JustWave::CACHE_FILENAME);
					// print_r($this->db);
			...
			$result = $this->db->query("SELECT filename, wavedata, progressdata, duration 
						FROM waves WHERE md5key='$md5' LIMIT 1");
	        $row=$result->fetch_array();
			list($filename, $waveData, $progressData, $this->duration) = $row;
	...
	private function cache()
	{
		...
		try {
			$stmt = $this->db->prepare("INSERT INTO waves
						VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			$timestamp = date('Y-m-d H:i:s');
            $stmt->bind_param("sssssssssss", $this->key, $this->url,
